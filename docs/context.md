...
Cập nhật: 2026-09-15, sau khi triển khai driver CAN0 và CanIf Part 1. Đọc cùng [codebase-map.md](codebase-map.md) trước mỗi task. Snapshot phải được kiểm tra lại nếu người dùng đang thay đổi repo.
...
- [src/main.c](../src/main.c) chứa board harness CanDrv+CanIf+PduR Tx loopback. BSP → Can_Init → Freeze bật LPB/clear SRXDIS → CanIf_Init → 4 PduR invalid requests → 27 case (Can_Write/CanIf_Transmit/PduR_ComTransmit, DLC 0..8). PduR.c thật xử lý Tx; hai capture callbacks ở đầu ra CanIf vẫn nằm trong main.c vì PduR Rx/confirmation chưa triển khai.
...
- Board harness main.c đã mở rộng cho PduR Tx (2026-09-15): PASS=2/FAIL=3; passedCases/txConfirmations/rxIndications=27, pdurPassedCases=9, pdurRejectedCases=4. Thêm txPath, pdurSourcePduId/dest/global và rejection result để debug. Bốn invalid requests: unknown COM ID, NULL descriptor, DLC9, NULL data/DLC1. PduR path kiểm tra BUSY propagated E_NOT_OK, caller bytes không bị sửa và loopback đúng payload; callbacks hiện vẫn ở CanIf boundary. Host simulation C stdin dùng modules thật pass config production và sparse COM ID17→CanIf ID0 cùng nhánh timeout/corruption/init/Freeze/callback lỗi. FLASH firmware strict ARM compile/link với PduR.c/PduR_Cfg.c thật pass; ELF vẫn build/canif/CanIf_loopback.elf, không còn undefined symbols. Logs main_loopback_host_debug.log và main_loopback_build.log. Chưa board runtime; Freeze/poll waits bounded iterations, BSP waits cũ unbounded. Chỉ sửa main.c/core memory, không tạo source/header/test file mới.
...
- CanIf cung cấp hai callbacks cho CanDrv. PduR hiện có PduR_ComTransmit và Tx/Rx config; chưa có PduR_CanIfRxIndication/TxConfirmation, không có Init API. Main giữ capture callbacks cho hai API còn thiếu, vì vậy chỉ kiểm chứng PduR Tx routing. COM hiện chỉ có types/config, chưa Com.c/h. BSP/Can_Init chạy trước CanIf_Init; harness bật loopback qua Freeze. Không chạy API đồng thời hoặc xử lý cùng MB bằng IRQ.
