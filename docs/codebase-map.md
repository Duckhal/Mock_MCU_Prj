...
## Interactive UART/CanTp paths (2026-09-22)

```text
Tx PC UART TX -> LPUART1 Rx ISR -> app UART ring -> App_SendLargeMessage
  -> NodeApp -> PduR -> CanTp -> CanIf -> CanDrv -> physical CAN0 bus
  -> CanDrv -> CanIf -> PduR -> CanTp -> NodeApp READY queue
  -> Rx App_MainFunction -> blocking raw-byte output to Rx PC UART
```

`app/app.c` owns stream collection, the 20 ms chunk gap, the 62-byte boundary,
role isolation, UART delivery and the 500 ms application timeout. Exactly one
physical board is selected as Tx; the other boards stay in Rx. `src/main.c`
does not inspect UART or N-SDU data. `SYSTEM_ENABLE_UART_CANTP_LOOPBACK=1U`
selects the optional single-board test path; keep it `0U` for external CAN
traffic. Host/ARM evidence is generated by `tests/board_demo/run_tests.ps1`
under `build/uart_cantp_echo/`.
